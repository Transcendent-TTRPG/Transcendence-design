# Zarnag

**Status:** In progress  
**Animal base:** Hyena  
**Mythic source:** Horn of Africa hyena-death and hyena-witch folklore  
**Primary reference:** [index.md](index.md)  
**System connections:** `docs/system/natural-attack-forms.md`, `docs/system/technique-origins.md`, `docs/system/backgrounds.md`

---

## 1. Identity

The Zarnag are feared because they go where the dead have not finished leaving.
They are sought for the same reason.

Other peoples speak of them as corpse-eaters, plague walkers, grave defilers,
or night scavengers. The Zarnag do not define themselves by desecration. They
define themselves by what they understand that others do not: the dying leave
their wanting in what they held. Fear, search, grief, rage — whatever the
dying did not finish passes into the objects they carried. If no one receives
that weight, it pulls at the living who come near.

Their oldest civilizational claim is not that they do what others refuse. It
is that they are the only ones who learned what to do with what the dead leave
behind.

They are not a people of abstract darkness. They are a people of unfinished
wanting, dangerous weight, and the work of closure.

---

## 2. Body

Zarnag are medium humanoids descended from hyena stock, with sloped
shoulders, powerful necks, dense torsos, long forearms, heavy jaws, and a gait
that seems permanently ready to lower, lunge, drag, or circle. Their bodies
suggest endurance under bad conditions more than elegance or grace. They are
built for unstable ground, sudden violence, dragging burdens, tearing through
resistant matter, and lingering in places that would make others retch or
flee.

Their jaws are among their most defining features. A Zarnag bite is not
simply a natural weapon; it is a civilizational symbol of breach, contact, and
consequence. Their claws are shorter and more practical than theatrical:
useful for scrambling, digging, raking, gripping, and opening weak seams in
hide, cloth, wood, or flesh.

They possess strong tolerance for foul air, rot, blood mist, and the chemical
signatures of decay. This does not make them invulnerable to disease, but it
does mean they survive and interpret environments that would quickly sicken or
unsettle other peoples. Their laughter, too, is bodily before it is symbolic:
it is a stress signal, a pressure display, a way to harden group nerve in bad
places, and sometimes an intimidation instrument that puts others on edge even
before violence begins.

### Physical baseline

- **Height:** `1.61 - 1.73 m`
- **Weight:** `65 - 73 kg`
- **Lifespan:** `50 - 70 years`
- **Size category:** `Medium`

### Visible variation

Different Zarnag lineages and camps may emphasize heavier jaws, lighter frames,
rougher coats, stronger carrion tolerance, harsher laughter, or eyes better
adapted to low light and grave smoke. These differences matter socially and
ecologically, but do not yet imply rigid biological castes.

---

## 3. Culture

Zarnag culture formed around a single inherited observation: the dying leave
their wanting in what they held. Fear, search, grief, unfinished purpose —
whatever consciousness did not resolve passes into objects at the moment of
death. If no one receives that weight, it pulls at the living who approach the
site. In places where many died, the accumulation is not metaphorical. It is
measurable in what happens to people who enter without preparation.

Their central civilizational idea is **the weight that remains**. Battlefield
objects, possessions of plague dead, items left in failed burials, things
found in ruins — these are not passive remnants. They carry the unfinished
wanting of whoever held them last. A settlement that ignores this is not pure.
It is simply delaying the consequences.

Because of this, Zarnag communities specialize in what they call closure work:

- battlefield stripping and object recovery
- plague disposal and quarantine trench work
- grave correction and reburial
- ruin clearance where the dead are old and disturbed
- scavenger-route management where weight has accumulated at kill sites

Respect inside Zarnag society does not come from innocence. It comes from
stamina, nerve, and the precision to distinguish weight that can be received
from weight that cannot — and to know when to burn rather than carry.

Laughter plays a specific role in this culture that outsiders misread as
bravado. Near the dead, laughter is an assertion of presence: we are here,
we still want, we are alive — what you left cannot claim us. It is a way of
holding against the pull of accumulated wanting. But the Zarnag know that
this same laughter can cross into something else: the signal that someone is
no longer asserting life against the weight, but beginning to enjoy the weight
itself. Many of their moral warnings revolve around that moment.

---

## 4. Theology and Cosmology

Zarnag cosmology begins from one shared observation: the dying leave their
wanting in what they held. Different communities name it differently — corpse
hunger, grave weight, what the Limbo did not collect when life closed — but
the underlying claim is the same. Whatever the dying did not finish passes
into their objects. If no one receives it, it pulls.

Burial, stripping, burning, sorting, sealing — none of these acts are purely
practical to the Zarnag. They are acts of reception: taking the weight that
remained, holding it long enough to neutralize it, returning to the world
something that no longer pulls. The most experienced practitioners know that
this weight leaves a mark in whoever carries it. Not always visible from
outside. Sometimes from inside, in ways that take years to become legible.

Their horror is not rooted in “darkness” as a generic force. It is rooted in
the specific possibility that the world has stopped being able to close its
dead — that battlefield trenches, plague caverns, ruined cities, and disturbed
ossuaries are evidence of a failure already in progress that no one else wants
to look at. Every site that resists closure, every object still wanting past
its time, is a data point.

The Zarnag do not ask whether death is frightening. They ask whether anything
truly leaves at all.

The elders call the defining moral threshold **the crossing**: the moment when
someone who received weight in order to close it began to need the weight to
exist in order to receive it. A community that has lived close to this work
for generations learns to recognize that moment in its members before the
member recognizes it in themselves. Every community has stories about what
happened when someone crossed and no one caught it in time.

---

## 5. Organization and Internal Variation

Zarnag communities tend to organize around the labor they perform and the risks
they accept. They are not one uniform grave-culture.

Common community forms include:

- **Grave-edge camps:** small, mobile, practical communities managing burial
  lines, scavenger trails, and local contamination.
- **Battlefield followers:** larger or tougher groups attached to recurring war
  zones, army routes, or regions where dead accumulate faster than they can be
  honored.
- **Plague-front settlements:** quarantine-oriented communities built around
  harsh sanitation, corpse sorting, and infection control.
- **Ruin-strip circles:** bands or enclaves that work old ruins, tombs, and
  catastrophic sites where the dead are old, layered, or disturbed.
- **Bone-yard polities:** more established Zarnag centers built around pits,
  ossuaries, rendering grounds, or ancestral fields with long memory and strong
  internal taboos.

Important roles may include grave walkers, carrion judges, fear breakers,
trackers, trench wardens, renderers, and bone keepers. None of these are fully
fixed castes yet, but they point toward internal specialization.

Their major fault lines are moral as much as political:

- service to outsiders vs refusal of outsider contempt
- cleansing the dead vs exploiting the dead
- hardening the community vs normalizing cruelty
- preserving dangerous sites vs burning them empty
- practical survival vs sacral duty

The Zarnag know that one of their greatest dangers is internal: a community can
become monstrous without noticing the moment discipline turns into appetite.

---

## 6. Daily Life and Survival

Zarnag life is harsh, dirty, and tense even by the standards of Transcendence.
Their camps and settlements often exist near spoilage, open graves, scavenger
territory, unstable ruins, bad water, or the fringes of other peoples who want
their labor without wanting them close.

Food is practical before it is refined: dried meat, rendered fat, bitter roots,
broths, fungi, marrow, smoked offal, salvaged stores, and whatever can be
secured without crossing into corruption or disease. Knowing whether something
is merely foul or truly dangerous is a basic survival skill.

Children grow up learning things other peoples would rather never know:

- which flies mean fever and which mean ordinary rot
- which smells suggest opened graves
- what kind of silence means scavengers are near
- how quickly bad rain turns pits dangerous
- when laughter steadies a group and when it means someone is losing restraint

Initiation often revolves around judgment rather than bravery alone. It is not
enough to walk into a terrible place. One must return having interpreted it
correctly.

Daily fear is everywhere:

- a burial trench reopening after weather
- a scavenger swarm following the wrong scent toward camp
- a corpse field turning to sickness
- a ruin cache proving less dead than it looked
- a child misreading danger
- a neighbor settlement breaking contract and abandoning its dead nearby
- someone in the community beginning to enjoy the work too much

Old age is respected when it preserves judgment. Elder Zarnag are valued not
for softness, but for knowing what can be survived, what only looks survivable,
and which forms of contamination reach the spirit through habit rather than
wound.

---

## 7. External Relations

Few peoples encounter the Zarnag neutrally. Most fear them before understanding
them. Many also need them.

When plague pits open, when battlefields rot, when burial grounds fail, when
scavengers gather too close, when a ruin is thick with old remains, the Zarnag
become necessary. Yet this necessity rarely produces comfort or trust. They are
often hired reluctantly, paid badly, blamed quickly, and sent away once the
labor is done.

This gives them a recurring political position across the world: pushed to the
edge, summoned back when the edge spills inward.

Typical external pressures include:

- monstrous ecologies drawn to rot, fear, or exposed remains
- raids against camps burdened with corpse labor
- trade exclusion from settlements that still rely on Zarnag services
- theological hostility from peoples who read grave-work as defilement
- uneasy dependence from war bands, frontier towns, and ruin economies
- recurring suspicion that Zarnag benefit from the crises they manage

The Zarnag are therefore shaped by a contradiction: they are useful exactly
where others most want distance.

**To the Naghii:** The Naghii usually recognize Zarnag value in places where ruins and the dead mix, but rarely trust their method. To a Naghii, a Remnant must be read within a chain of meaning, inscription, sky, and archive. To a Zarnag, it must first be determined if it will poison the water, raise scavengers, bite the living, or leave active Weight on the site. This difference produces useful cooperation and mutual contempt: the Naghii can seem too slow; the Zarnag, too willing to touch before understanding.

**To the Sauri:** The Sauri understand better than most the idea of handling what others prefer not to see. But their relationship with preservation can be unsettling for the Zarnag. A preserved body can be a record, authority, or warning. It can also be something that should have been satiated and was retained out of pride, lineage, or theology. Where the Sauri ask what can be contained, the Zarnag ask what is happening because it is kept here.

**To the Panin:** The Panin are a practical problem. In zones of death, infection, or active Remnants, their curiosity can break perimeters before they understand the perimeter exists for a reason. The Zarnag don't usually hate them; they just get exhausted quickly around them. A Panin touching what they shouldn't can turn a difficult day into a crisis. A Panin who listens, however, can be useful for tracking routes, entrances, and small environmental changes others miss.

**To the Rokhart:** The Rokhart document and the Zarnag remember with the body. They can work well together in disaster fields, provided the Rokhart understands that not everything should be preserved as data before being treated as a risk. Tension appears when someone wants to register a site that is still active, move pieces before satiating the perimeter, or take evidence the Zarnag consider contaminated.

**To the Myo:** The Myo treat the Zarnag with a different caution. Where the Zarnag read the material Remnant, the Myo can read echoes, omens, or patterns that do not depend on the physical body. Some Zarnag circles respect that ability. Others consider it a dangerous way of staying too close to what should be satiated.

**To the Drak'kai:** The Drak'kai are especially problematic in ancient death sites. A Remnant found among old dead is not simply an interesting object for the Zarnag. It is part of a scene that remains hungry until proven otherwise. The Drak'kai might want to keep, study, or claim. The Zarnag might want to burn, seal, or bury deep. More than one conflict has started with both convinced they are preventing a greater loss.

**To the Loxod:** The Loxod can receive layers of grief, fear, death, waiting, and permanence in a site before the Zarnag finish their practical reading. This can be both useful and dangerous. A Loxod can say what emotional charge sustains the Weight. A Zarnag can reply that knowing it too early doesn't help if no one has closed the perimeter. The best work between both species happens when the Loxod reads without appropriating and the Zarnag satiates without denying what was read.

**To the Ceratox:** The Ceratox detect territorial pressure where the Zarnag detect accumulated Hunger. In mass death sites, their readings can overlap in unsettling ways. A Ceratox might feel an area is too heavily charged to cross; a Zarnag might know which Remnant is producing that charge. Cooperation is valuable, but difficult: an alert Ceratox can make a perimeter that is already close to biting even tenser.

**To the Ursari:** The Ursari understand the cost of years of exposure to a craft that changes the body and judgment. The forge and the mass grave are not the same, but both teach that working close to something powerful leaves a mark. The Ursari usually respect Zarnag discipline more than other species. The Zarnag, for their part, distrust any tradition that sees prolonged exposure as a source of authority without asking frequently enough what part of the worker is still their own.

**To the Arakhel:** The Arakhel are useful interlocutors in places where paths, crossings, and deaths overlap. An ancient node near a battlefield might know who passed before the grave opened. An Arakhel family can distinguish transit from permanence where the Zarnag distinguish Remnant from waste. But the Arakhel sometimes preserve crossings the Zarnag would prefer to close, and the Zarnag sometimes burn or seal things the Arakhel consider part of a reading of the in-between.

**To the Bufoni:** The Bufoni matter especially when a domain falls. To the Bufoni, a kunitsukami without attention can continue responding without vocabulary, protocol, or community to recognize it. To the Zarnag, a fallen domain is usually full of Remnants: offerings, soil, bodies, names, and territorial Last Hungers no one finished satiating. The exchange can save survivors or open deep disputes about whether something should be restored or closed.

**To the Formix:** The Formix offer refuge in a way many Zarnag don't expect. A free Formix community can receive marginalized workers without carrying the same historical contempt for their craft. But they can also ask uncomfortable questions: if a Zarnag exists to work the Hunger, what remains of them when there is no Hunger around? That question can sound too much like Gluttony to some old Zarnag.

**To the Chelicer:** The Chelicer find it difficult to accept that such an important craft depends so much on dirty, situated, bodily judgment. For a Chelicer, truth must be revealed under procedure. For a Zarnag, procedure only serves if someone can still smell when the site changed. Both understand the value of the correct conditions, but disagree on what happens when those conditions keep producing something that shouldn't be there.

---

## 8. System Connections

### Competency affinities

Likely Zarnag strengths include:

- survival under contamination or scarcity
- corpse handling and practical anatomy
- intimidation and morale pressure
- tracking through foul or broken environments
- infection resistance and triage judgment

### Natural attack logic

Their current natural-weapon identity should center on:

- **Bite:** close breach, seizure, infection risk, and consequence after
  contact
- **Claws:** fast raking, chaining blows, scrambling, and tearing

### Arsenal fit

Likely weapon affinities:

- daggers and short blades
- axes, cleavers, and utilitarian cutters
- thrown tools
- natural bite and claws

Possible but less central:

- polearms for carcass distance or battlefield work
- shields for trench or plague-front duty
- heavy armor only in specialized roles

### Candidate species mechanics

Future Zarnag mechanics might explore:

- carrion tolerance
- twisted laughter as pressure signal
- corpse-field judgment
- contamination risk management

### Technique implications

Zarnag-origin Techniques should emerge from:

- grave labor
- survival around remains and contamination
- fear discipline
- scavenger-route reading
- infection threat after contact
- practical judgment under disgust

They should avoid collapsing into generic shadow magic or flavorless “dark
ritual” powers. Their horror is stronger when it comes through rot, remains,
threshold labor, and the unbearable fact that someone has to touch what others
abandon.

---

## 9. Design Notes

### What makes them distinct

The Zarnag should not be “dark hyena people.” Their core identity is that they
survive and build culture where death, refuse, contamination, and social taboo
become unavoidable labor.

### What to avoid

- direct copying of a real human culture
- generic cult-of-darkness aesthetics
- reducing them to villains
- making them simply plague-immune
- treating carrion and death only as style rather than survival logic

### Open questions

- What do the Zarnag believe happens when a corpse is neither properly consumed
  nor properly buried?
- Which Zarnag communities are more feared: battlefield followers, plague-edge
  camps, or ruin-strip circles?
- Do some Zarnag see themselves as protectors of the living, while others
  increasingly identify with the dead and the remainder?
- What kinds of ancient ruins or catastrophic zones first taught them that bad
  death can spread?
